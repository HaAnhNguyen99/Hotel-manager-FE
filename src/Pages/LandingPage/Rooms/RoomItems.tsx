...
const RoomItems = () => {
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-10 md:gap-5 md:justify-between dark:bg-transparent mt-20 lg:max-w-[80%] lg:mx-auto lg:gap-10">
      {data.map((item, index) => (
        <div
          key={index}
          className="h-fit  shadow-medium w-full rounded-2xl overflow-hidden">
          <div className="h-[50vh] sm:h-[30vh] md:h-[40vh] lg:h-[50vh] w-full flex flex-col bg-landing-pureWhite  rounded-2xl overflow-hidden pb-6">
            <div
              className="h-4/5 mb-2 flex-1"
              style={{
                background: `linear-gradient(
                0deg,
                rgba(240, 230, 210, 0.2) 0%,
                rgba(240, 230, 210, 0.2) 100%
              ), url(${item.src}) lightgray 50% / cover no-repeat`,
              }}></div>
