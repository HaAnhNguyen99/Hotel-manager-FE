    ...
    <Popover open={isOpen} onOpenChange={setIsOpen} modal>
      <PopoverTrigger asChild>
        <Button
          ...
          {date ? format(date, 'MM/dd/yyyy HH:mm', { locale: vi }) : <span>MM/DD/YYYY HH:mm</span>}
        </Button>
      </PopoverTrigger>
              ...
              <div className="flex sm:flex-col p-2">
                {hours.reverse().map((hour) => (
                  <Button
                    ...
                    className="sm:w-full shrink-0 aspect-square dark:text-zinc-400 dark:hover:text-zinc-400"
              ...
              <div className="flex sm:flex-col p-2">
                {['Sáng', 'Chiều'].map((ampm) => (
                  <Button
                    key={ampm}
                    size="icon"
                    variant={date && ((ampm === 'Sáng' && date.getHours() < 12) || (ampm === 'Chiều' && date.getHours() >= 12)) ? 'default' : 'ghost'}
