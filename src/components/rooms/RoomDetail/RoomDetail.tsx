...
const DetailRow = ({ icon, label, value, isStatus = false }: DetailItem) => (
  <p className="flex items-center gap-1">
    ...
    <span className={`font-bold ${isStatus ? 'ml-1 p-1  shadow-md  bg-white text-green-900 rounded-lg text-[0.8rem] font-normal' : ''}`}>
      {value}
    </span>
  </p>
);
...
export const RoomDetails = ({ room }: { room: Room }) => {
  ...
  return (
    <section className="space-y-4 text-sm">
      ...
      {/* Basic Info Section */}
      <div className="p-2 py-4 w-full border border-[rgb(228,228,231)] rounded-2xl space-y-2 shadow-lg dark:shadow-md shadow-zinc-200 dark:shadow-slate-400">
